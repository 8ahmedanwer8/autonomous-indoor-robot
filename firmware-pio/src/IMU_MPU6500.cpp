...
void printWhoAmI()
{
  Serial.print(F("MPU6500 WHO_AM_I at 0x68 = 0x"));
  if (g_whoAmI < 16)
  {
    Serial.print('0');
  }
...
bool setupImuMpu6500()
{
  ...
  {
    Serial.println(F("MPU6500 WHO_AM_I read failed"));
    scanI2cBus();
    g_imuReady = false;
    return false;
  }
  ...
  {
    Serial.println(F("Unsupported MPU6500-class IMU type"));
    g_imuReady = false;
    return false;
  }
  ...
  {
    Serial.println(F("MPU6500 register configuration failed"));
    g_imuReady = false;
    return false;
  }
  ...
  for (int i = 0; i < GYRO_CAL_SAMPLES; i++)
  {
    ...
    if (!readImuSample())
    {
      Serial.println(F("MPU6500 gyro calibration failed"));
      setStatusLed(false);
      g_imuReady = false;
      return false;
    }
  ...
  Serial.println(F("MPU6500 IMU ready"));
  Serial.print(F("MPU6500 gyro bias z rad/s: "));
